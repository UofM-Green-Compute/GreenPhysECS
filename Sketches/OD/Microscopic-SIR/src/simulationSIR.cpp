...
void setupEntities(flecs::world world, std::vector<flecs::entity> &p, int totalPopulation, 
    ...
    for (int i = 0; i < initialInfected; ++i) { 
        ...
        p.push_back( 
            world.entity() 
                .set<PeopleIndex>({totalPopulation-initialInfected-initialRecovered + i})
                .set<State>({2})
                .set<TransitionProbabilities>({Qnm})
        ); 
    }
    for (int i = 0; i < initialRecovered; ++i) { 
        ...
        p.push_back( 
            world.entity() 
                .set<PeopleIndex>({totalPopulation-initialRecovered + i})
                .set<State>({3})
                .set<TransitionProbabilities>({Qnm})
        ); 
    }
...
const int totalPopulation, const int initialInfected, const int initialRecovered, const double step, const double totalTime, 
std::string filename) {
    /*
    This function
    ...
    */
    ...
    setupEntities(world, people, totalPopulation, initialInfected, initialRecovered); 
    ...
    MyFile << "Time,Susceptible,Infected,Recovered" << std::endl; 
    double time = 0;
    MyFile << time << "," << population_vector[0] << "," << population_vector[1] << "," << population_vector[2] << std::endl;  

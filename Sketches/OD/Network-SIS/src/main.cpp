        ...
        for(int k = 0; k < NO_PEOPLE; k++){
            ...
            if ((rand < c) && (i != k) && (population[i].get<State>().s != 2) && (population[k].get<State>().s != 2)) { 
                ...
                MyFile << population[i].get<peopleIndex>().i << "," << k << ";" ; 
                    ...
                    ((population[i].get<State>().s) == 2 || (population[k].get<State>().s == 2))) { 
...
int main(int argc, char* argv[]) {
    ...
    world.system<peopleIndex, State, infectedConnections, TransitionProbabilities>()
        .each([&](flecs::entity e, peopleIndex& person, State& state, infectedConnections infected,TransitionProbabilities& p_vector){
            ...
            for(int l = 0; l < NO_PEOPLE; l++){
                if( e.has(infectionLink,population[l]) || e.has(Link,population[l])) { 
                    MyFileEdges << person.i << "," << l << ";"; 
                }
            }
    ...
    world.system<peopleIndex, State, infectedConnections, TransitionProbabilities>()
        .each([&](flecs::entity e, peopleIndex& person, State& state, infectedConnections infected,TransitionProbabilities& p_vector){
            ...
            if( state.s == 1 ){
                p_vector.qnm = {1 - (beta * infected.no),beta * infected.no}; 
            }
            else if ( state.s == 2 ) {
                p_vector.qnm = {alpha,1 - alpha}; 
            }
    ...
    world.system<peopleIndex, State, TransitionProbabilities>()
        .each([&](flecs::entity e, peopleIndex& person, State& state, TransitionProbabilities& p_vector){
            ...
            for(int j = 0; j < NO_PEOPLE; j++){
                ...
                else if( (person.i != j) && ((state.s == 2) || (population[j].get<State>().s == 2)) && !(population[j].has(infectionLink,e)) && (e.has(Link,population[j]))) 
                { 
                    e.remove(Link,population[j]);
    ...
    for(int j = 0; j < TIME_STEPS; j++){
        MyFileEdges << j << ";" ;
        MyFileNetworkSIR << j << "," << population_sizes[0] << "," << population_sizes[1] << std::endl; 
        world.progress(); 

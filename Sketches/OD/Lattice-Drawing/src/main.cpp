...
std::vector<std::vector<double>> setupLattice(int totalPopulation, std::string filename, std::string filename2, double a, int worked_counter){
    ...
    while ( (y >= 0) && (y <= Y_LIM) ){
        ...
        while( (x >= -epsilon) && x <= (X_LIM) ){
            ...
            // Only write the required no of points 
            if(count2 < totalPopulation){
                MyFile << x << "," << y << "|" ;  
    ...
    if (count < totalPopulation){ return setupLattice(totalPopulation, filename, filename2, 0.5 * a, worked_counter); } 
    if (count >= totalPopulation && worked_counter < 1500){ worked_counter+=1; return setupLattice(totalPopulation, filename, filename2, 1.5 * a, worked_counter); }
    else{ return GridPoints; }
...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    // Draw the lattice
    std::vector<std::vector<double>> GridPoints = setupLattice(totalPlants, FILENAME, FILENAME2, X_LIM, 0); 
